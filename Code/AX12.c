...
Spring 2013 - TRANSLATED TO ENGLISH
MJ & EB		  Adapted for LPC1114 and TRM system
...
{	
    ...
    sendData(AX_ACTION);
    sendData(AX_ACTION_CHECKSUM);	//250
	delay32Us(1,TX_DELAY_TIME);
	switchCom(Rx_MODE);
}
...
/* Included Fall 2013 by Danqi Ye */
/*
int AX12_getID()
{
	Checksum = (~(BROADCAST_ID + AX_RDT_LENGTH + AX_READ_DATA + AX_ID + AX_ID_LENGTH))
	switchCom(Tx_MODE);
	sendData(AX_START);
	sendData(AX_START);
	sendData(BROADCAST_ID);
